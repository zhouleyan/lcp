...
import { listRoleBindings, createRoleBinding, deleteRoleBinding, listRoles } from "@/api/iam/rbac"
...
function CreateRoleBindingDialog({
  ...
  const filteredUsers = searchQuery
    ? users.filter((u) => {
        ...
        return u.spec.username.toLowerCase().includes(q) || u.spec.email?.toLowerCase().includes(q) || u.spec.displayName?.toLowerCase().includes(q)
      })
    : users
  ...
  const handleSubmit = async () => {
    setSubmitting(true)
    try {
      await createRoleBinding({ spec: { userId: selectedUserId, roleId: selectedRoleId, scope: "platform" } })
      ...
      <DialogContent onOpenAutoFocus={(e) => e.preventDefault()}>
        ...
        <div>
          ...
          <div className="relative mt-1">
            ...
            <Input placeholder={t("user.searchPlaceholder")} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="pl-9" />
          </div>
...
export default function RoleBindingListPage() {
  ...
  const fetchBindings = useCallback(async () => {
    ...
    try {
      ...
      const data = await listRoleBindings(params)
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteRoleBinding(deleteTarget.metadata.id)
  ...
  const handleBatchDelete = async () => {
    try {
      await Promise.all(Array.from(selected).map((id) => deleteRoleBinding(id)))
      toast.success(t("action.deleteSuccess"))
  ...
  return (
    <div className="p-6">
      <div className="mb-6 flex items-center justify-between">
        ...
        {hasPermission("iam:rolebindings:create") && (
      ...
      <div className="mb-4 flex items-center gap-3">
        ...
        {selected.size > 0 && hasPermission("iam:rolebindings:delete") && (
      ...
      <div className="border">
        <Table>
          <TableHeader>
            <TableRow>
              {hasPermission("iam:rolebindings:delete") && (
