...
ORDER BY
    ...
    CASE WHEN sqlc.arg('sort_field')::VARCHAR = 'member_count' AND sqlc.arg('sort_order')::VARCHAR = 'asc' THEN (SELECT count(*) FROM user_namespaces un WHERE un.namespace_id = ns.id) END ASC,
    CASE WHEN sqlc.arg('sort_field')::VARCHAR = 'member_count' AND sqlc.arg('sort_order')::VARCHAR = 'desc' THEN (SELECT count(*) FROM user_namespaces un WHERE un.namespace_id = ns.id) END DESC,
    ns.created_at DESC
